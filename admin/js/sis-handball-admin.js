(function ($) {
    ...
    $(document).ready(function () {
        ...
        updateLinkChecker('http://www.sis-handball.de/');
    ...
    function updateFieldView() {
        ...
        if (select_field.val() === 'standings' || select_field.val() === 'chart' || select_field.val() === 'stats') {
            $('#sis_handball_shortcode_generator_league_id_description_link').html('http://sis-handball.de/default.aspx?view=Tabelle&Liga=<code>001517000000000000000000000000000001000</code>');
            updateLinkChecker('http://sis-handball.de/default.aspx?view=Tabelle&Liga=', league_id);
        }

        if (select_field.val() === 'team' || select_field.val() === 'next') {
            $('#sis_handball_shortcode_generator_league_id_description_link').html('http://sis-handball.de/default.aspx?view=Mannschaft&Liga=<code>001517000000000000000000000000000001015</code>');
            updateLinkChecker('http://sis-handball.de/default.aspx?view=Mannschaft&Liga=', league_id);
        }

        if (select_field.val() === 'games') {
            $('#sis_handball_shortcode_generator_league_id_description_link').html('http://sis-handball.de/default.aspx?view=AlleSpiele&Liga=<code>001517000000000000000000000000000001000</code>');
            updateLinkChecker('http://sis-handball.de/default.aspx?view=AlleSpiele&Liga=', league_id);
        }

        /* Update visible fields */
        if (select_field.val() === 'standings' || select_field.val() === 'team' || select_field.val() === 'games' || select_field.val() === 'stats') {
            allow_marked = 1;
    ...
    function updateConcatenationFieldView() {
        $('#sis_handball_concatenation_league_id_description_link').html('http://sis-handball.de/default.aspx?view=Mannschaft&Liga=<code>001517000000000000000000000000000001015</code>');
    }
    ...
    $('body').on('click', '#sis_handball_shortcode_generator_fetch_teams', function () {
        var type = $('#sis_handball_shortcode_generator_type').val();
        var league_id = $('#sis_handball_shortcode_generator_league_id').val();
        var marked_select_field = $('#sis_handball_shortcode_generator_marked');
        $.ajax({
            method: 'POST',
            url: "/wp-admin/admin-ajax.php",
            data: {action: 'team_fetch', type: type, league_id: league_id},
            beforeSend: function () {
